...
export async function getRepeatableJobs() {
  return (await JQueue.getRepeatableJobs())
    .map(job => ({
      ...job,
      id: job.id || '',
      flag: false,
...
const getInnerJobProps = (job) => {
  ...
  if (job.name === '__default__' && job.data.job.description) job.name = job.data.job.description;
...
export async function getJobs(params: IGetTaskParams): Promise<{ repeatable: any[], jobs: any[] }> {

  const repeatable = await getRepeatableJobs();

  const jobsStatus = !params.Status || params.Status === 'all' ? allJobsStatus : [params.Status];
  const jobs: any[] = [];

  for (const status of jobsStatus) {
    let jobsByStatus = (await JQueue.getJobs([status as any])).filter(e => e);
    if (params.StartDate && params.EndDate) {
      const dates = { start: params.StartDate?.valueOf(), end: params.EndDate?.valueOf() };
      jobsByStatus = jobsByStatus.filter(e => (!e.processedOn || (e.processedOn && e.processedOn > dates.start))
...
export async function removeJobsALL(): Promise<void> {
  ...
  const allJobsCodes = (await JQueue
    .getJobs(allJobsStatus))
    .filter(e => e)
    .map(e => e.id);
