...
export class SQLClient {
	...
	private setParams(params: any[], request: Request) {
		for (let i = 0; i < params.length; i++) {
			...
			} else request.addParameter(`p${i + 1}`, TYPES.NVarChar, params[i]);
		}
	}
	...
	private parsingParamsSQL(sql: string, params: any[]) {
		...
		for (let i = 0; i < params.length; i++) {
			...
			if (params[i] instanceof Date) {
				...
				if (params[i]) ps = 'cast(1 as bit)';
				else ps = 'cast(0 as bit)'; // ???
			} else ps = `'${params[i].toString()}'`;
		...
		return new Promise<T[]>(async (resolve, reject) => {
			try {
				...
				const connection = this.connection
					? this.connection
					: await this.sqlPool.pool.acquire().promise;
				const request = new Request(
					sql,
					(error: RequestError, rowCount: number, rows: ColumnValue[][]) => {
	...
	async manyOrNoneStream(
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				const request: Request = new Request(parsingParamsSQL, (error) => {
					if (error) return reject(error);
				});
				request.on('row', (row) => onRow(row, request));
				request.on('done', async (rowCount: number, more: boolean) => {
					await onDone(rowCount, more);
					resolve();
				});
	...
	async oneOrNone<T>(sql: string, params: any[] = []): Promise<T | null> {
		return new Promise(async (resolve, reject) => {
			try {
				...
				const request = new Request(
					sql,
					(error: RequestError, rowCount: number) => {
						if (!this.connection) this.sqlPool.pool.release(connection);
						if (error) return reject(error);
	...
	async tx(
		...
		try {
			await func(
				new SQLClient(this.sqlPool, this.user, connection),
				name,
				isolationLevel,
			);
	...
	private beginTransaction(
		...
		name?: string,
		isolationLevel: ISOLATION_LEVEL = ISOLATION_LEVEL.READ_COMMITTED,
	): Promise<this> {
		return new Promise(async (resolve, reject) => {
	...
	private toJSON(value: any): any {
		if (
			...
			try {
				return JSON.parse(value, dateReviverUTC);
			} catch {
				return value;
			}
		else return value;
