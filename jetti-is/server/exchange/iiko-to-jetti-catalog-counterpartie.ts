...
///////////////////////////////////////////////////////////
const newCounterpartie = (syncParams: ISyncParams, iikoCounterpartie: IiikoCounterpartie) => {
  return {
...
async function syncCounterpartie(syncParams: ISyncParams, iikoCounterpartie: IiikoCounterpartie, destSQL: SQLClient): Promise<any> {
  let response = await GetCatalog(iikoCounterpartie.project, iikoCounterpartie.id, iikoCounterpartie.baseid, 'Counterpartie', destSQL);
  if (response === null) {
...
export async function ImportCounterpartieSQLToJetti(syncParams: ISyncParams) {
  ...
  await ssql.manyOrNoneStream(`
    ...
    --  and spr.id = '0339CD20-594B-4CCC-AF7A-00268C2A8C11'`, [syncParams.lastSyncDate],
