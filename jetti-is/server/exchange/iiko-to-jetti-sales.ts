...
async function syncSalesSQL(
	...
	if (syncParams.logLevel > 1)
		await saveLogProtocol(
			...
			0,
			syncStage,
			`Session ${iikoDoc.id} ${iikoDoc.dateIncoming.toString()} #${
	...
	if (typeFranchise.tf === 'Classic franchise')
		Operation = '42C3BE10-1831-11EA-822C-15390275940A'; // другая операция по франшизе
	const customer: any = await destSQL.oneOrNone(
		`SELECT id FROM [dbo].[Catalog.Counterpartie.v] WITH (NOEXPAND) where [Department] = @p1 and [Client]='true' `,
		[store.doc.Department],
	); // розничный покупатель
	const company2: any = await destSQL.oneOrNone(
		`select cast(dbo.Company2OnDate(@p1, @p2) as nvarchar(50)) as id;`,
		[iikoDoc.dateIncoming, store.doc.Department],
	); // организация-2
	...
	if (!company2 === null) {
      ...
      order by [isDefault] desc, [code]`,
			[store.doc.Department, company2.id],
		); // параметры по организация2
		if (!params2 === null) {
    ...
    and ise.deletedWithWriteoff=2 and ise.writeoffpaymenttype is null `,
		[iikoDoc.id],
	);
	// оплаты
    ...
    where ise.session_id = @p1
      and not ise.orderId in (select DISTINCT i.orderId from dbo.ItemSaleEvent i where i.session_id = ise.session_id
        and i.deletedWithWriteoff=2 and i.writeoffpaymenttype is null) `,
		[iikoDoc.id],
	);
	// доставка
    ...
    left join dbo.OrderPaymentEvent p on p.[order] = i.orderId
    left join dbo.Delivery d on d.orderId = i.orderId `,
		[iikoDoc.id],
	);
	// кассы
	const CashRegister: any = await destSQL.oneOrNone(
		`SELECT top 1 d.id FROM [dbo].[Catalog.CashRegister.v] d WITH (NOEXPAND)
	...
	let CashRegister2: any = await destSQL.oneOrNone(
		`SELECT top 1 d.id FROM [dbo].[Catalog.CashRegister.v] d WITH (NOEXPAND)
	...
	// экв. терминал
	const AcquiringTerminal: any = await destSQL.oneOrNone(
		`SELECT top 1 d.[id], d.[BankAccount], d.[Counterpartie] as Acquier
    FROM [dbo].[Catalog.AcquiringTerminal.v] d WITH (NOEXPAND)
    where d.[Department] = @p1 and d.[company] = @p2  order by d.[isDefault] desc, d.[code]`,
		[store.doc.Department, company.id],
	);
	...
	for (const ord of Orders) {
      ...
      `,
        ...
        SELECT N'Operation ('+d.description+N') #'+@p1+N', '+convert(nvarchar(30), @p2, 127) as dsc FROM [dbo].[Catalog.Operation.v] d WITH (NOEXPAND) where d.[id] = @p3 `,
			...
			const managerz: any = await GetCatalog(
				syncParams.project.id,
				ord.cashier,
				syncParams.source.id,
				'Manager',
      ...
      `,
				[
					...
					docBankAccount,
					docBankAccount2,
					nullOrID(CashRegister),
					nullOrID(CashRegister2),
					docAcquiringTerminal,
					docAcquiringTerminal2,
					JSON.stringify(cashz),
					JSON.stringify(tabPayAdd),
				],
			);
        ...
        FROM OPENJSON(@p1) WITH (
          ...
          [AcquiringTerminal] nvarchar(50)) as p) as i`,
				[JSON.stringify(cashz)],
			);
			// доставка
        ...
        `,
			...
			if (Amount.PayAggregator > 0 && isDelivery === 0)
				docDeliveryType = 'EXTERNAL';
			...
			if (!NoSqlDocument) {
				...
				if (syncParams.logLevel > 2) {
					await saveLogProtocol(
						...
						0,
						syncStage,
						`Order ${ord.orderId} #${ord.orderNum} insert`,
					);
				}
				...
				if (syncParams.logLevel > 2) {
					await saveLogProtocol(
						...
						0,
						syncStage,
						`Order ${ord.orderId} #${ord.orderNum} update`,
					);
				}
				ucnt++;
			}
	...
	for (const delord of delOrder) {
		const NoSqlDocument: any = await GetDocument(
			syncParams.project.id,
			delord.orderId,
			syncParams.source.id,
			'OrderDoc',
	...
	if (syncParams.logLevel > 1)
		await saveLogProtocol(
			...
			`Session #${
				iikoDoc.documentNumber
			} orders: insert ${icnt}, update ${ucnt}, delete ${dcnt}. Processing time ${
				(endd - startd) / 1000
			} s.`,
		);
}

export async function ImportSalesToJetti(
	syncParams: ISyncParams,
	docList: any[] = [],
) {
	await saveLogProtocol(
		...
		0,
		syncStage,
		`Start sync Sales Documents`,
	);
	if (syncParams.baseType === 'sql')
		await ImportSalesSQLToJetti(syncParams, docList);
}

export async function ImportSalesSQLToJetti(
	syncParams: ISyncParams,
	docList: any[] = [],
) {
	...
	if (docList.length === 0) {
		...
		sw = ` where ( cast(pr.dateIncoming as DATE) between '${DateToString(
			syncParams.periodBegin,
		)}' and '${DateToString(syncParams.periodEnd)}' `;
		if (syncParams.execFlag === 126 && syncParams.autosync) {
			sw += ` or (cast(pr.dateIncoming as DATE) >= '${DateToString(
				syncParams.firstDate,
			)}' and cast(pr.dateIncoming as DATE) < '${DateToString(
				syncParams.periodBegin,
			)}' and pr.dateModified >= '${DateToString(
		...
		async (row: ColumnValue[], req: Request) => {
			...
			if (batch.length === ssqlcfg.batch.max) {
				...
				for (const doc of batch)
					await syncSalesSQL(syncParams, doc, ssql, dsql);
				batch = [];
				req.resume();
			}
		},
