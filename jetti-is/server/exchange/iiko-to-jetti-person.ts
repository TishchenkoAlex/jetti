	...
	if (!response) {
		if (syncParams.logLevel > 1)
			await saveLogProtocol(
				...
				0,
				syncStage,
				`insert Pperson ${iikoPerson.name}`,
			);
		...
		if (syncParams.forcedUpdate) {
			if (syncParams.logLevel > 1)
				await saveLogProtocol(
					...
					0,
					syncStage,
					`update Pperson ${iikoPerson.name}`,
				);
	...
	if (response === null) {
		if (syncParams.logLevel > 1)
			await saveLogProtocol(
				...
				0,
				syncStage,
				`insert Manager ${iikoPerson.name}`,
			);
		...
		if (syncParams.forcedUpdate) {
			if (syncParams.logLevel > 1)
				await saveLogProtocol(
					...
					0,
					syncStage,
					`update Manager ${iikoPerson.name}`,
				);
...
export async function ImportPersonToJetti(syncParams: ISyncParams) {
	await saveLogProtocol(
		...
		0,
		syncStage,
		`Start sync Persons and Managers`,
	);
    ...
    `,
		...
		async (rowCount: number, more: boolean) => {
			if (rowCount && !more && batch.length > 0) {
				if (syncParams.logLevel > 0)
					await saveLogProtocol(
						...
						0,
						syncStage,
						`inserting tail ${batch.length} person`,
					);
				for (const doc of batch) {
					await syncPerson(syncParams, doc, dsql);
					doc.type = 'Manager';
			...
			await saveLogProtocol(
				...
				0,
				syncStage,
				`Finish sync Persons and Managers`,
			);
		},
	);
}
