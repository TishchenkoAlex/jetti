...
async function syncProduct(
	...
	let response: any = await GetCatalog(
		iikoProduct.project,
		iikoProduct.id,
		iikoProduct.baseid,
		'Product',
	...
	const ProductKind: any = await destSQL.oneOrNone(
		`
    SELECT id FROM [dbo].[Catalog.ProductKind.v] WITH (NOEXPAND) where [code]=@p1`,
		[iikoProduct.prodtype],
	);
	if (!response) {
		...
		if (syncParams.forcedUpdate) {
			if (syncParams.logLevel > 1)
				saveLogProtocol(
					...
					0,
					syncStage,
					`update Product ${iikoProduct.name} products`,
				);
			...
			response.doc.Unit = await GetExchangeCatalogID(
				iikoProduct.project,
				iikoProduct.unit,
				iikoProduct.baseid,
				'Unit',
			...
			if (batch.length === ssqlcfg.batch.max) {
				...
				if (syncParams.logLevel > 0)
					await saveLogProtocol(
						...
						0,
						syncStage,
						`inserting to batch ${countRows} products`,
					);
			...
			if (rowCount && !more && batch.length > 0) {
				if (syncParams.logLevel > 0)
					await saveLogProtocol(
						...
						0,
						syncStage,
						`inserting to batch ${batch.length} products`,
					);
				for (const doc of batch) await syncProduct(syncParams, doc, dsql);
			}
			await saveLogProtocol(
