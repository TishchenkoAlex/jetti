...
export async function getQueueInstanceAPIByQueueId(spi: ISyncParams): Promise<{ instance: AxiosInstance, token: string }> {
...
async function QueuePostJetti(sp: ISyncParams) {
  ...
  while (wf) {
    ...
    const docs: any[] = await dsql.manyOrNone(`
      ...
      group by [id]
      having min([Attempts]) = @p3 `, [bsz, sp.flow, ic]);
    ...
    if (docs.length > 0) {
      ...
      let pcnt = 0; let ecnt = 0;
      for (const row of docs) {
        await queueInstance.instance.get(`${sp.project.baseURL}/${sp.project.queuePostURL}/${row.id}`,
          {
            headers: { Authorization: queueInstance.token }, httpsAgent: new Agent({
              rejectUnauthorized: false
            })
          }).then(async (response) => {
            if (sp.logLevel > 2) await saveLogProtocol(sp.syncid, 0, 0, syncStage,
              `Posted document id=${row.id}. Status=${response.status}`);
            await dsql.none(`delete from [exc].[QueuePost] where id = @p1 `, [row.id]); // удаляем из очереди
            pcnt = pcnt + 1;
          }).catch(async (error) => {
            await saveLogProtocol(sp.syncid, 0, 1, syncStage, `Error posting document id=${row.id}: ${error.message}. Status=${error.response.status}. ${error.response.data}`);
            await dsql.none(`
              update [exc].[QueuePost]
              set [Attempts]=[Attempts]+1, [Message]=@p1
              where id = @p2 and date <= @p3`, [error.response.data, row.id, row.date]); // счетчик ошибок
            ecnt = ecnt + 1;
          });
      }
      if (sp.logLevel > 0) await saveLogProtocol(sp.syncid, 0, ecnt, syncStage,
...
export async function QueuePost(params: any) {
  try {
    ...
    await saveLogProtocol(syncParams.syncid, 0, 0, syncStage,
      `Post queue documents Jetti: ${syncParams.project.id}. Flow ${syncParams.flow}`);
    await saveLogProtocol(syncParams.syncid, 0, 0, syncStage, `Starting Batch ${syncParams.startTime.toString()}`);
    if (syncParams.project.baseURL !== '') {
      await QueuePostJetti(syncParams);
    } else await saveLogProtocol(syncParams.syncid, 0, 1, syncStage, `Error: no documents post module for project ${syncParams.projectid}`);
    syncParams.finishTime = new Date();
    await saveLogProtocol(syncParams.syncid, 0, 0, syncStage, `Finished Batch ${syncParams.finishTime.toString()}`);
