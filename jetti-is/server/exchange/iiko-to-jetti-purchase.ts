	...
	try {
		...
		if (syncParams.logLevel > 1)
			await saveLogProtocol(
				...
				0,
				syncStage,
				`Purchase ${iikoDoc.id} ${iikoDoc.date.toString()} #${
		...
		if (contrAgent.isInternal === 1) {
			await saveLogProtocol(
				syncParams.syncid,
				0,
				0,
				syncStage,
				`log: Contr Agent isInternal  ${
					iikoDoc.id
				} ${iikoDoc.date.toString()} #${iikoDoc.documentNumber}`,
			);
		}
		...
		PositionPurchase = await exchangeManyOrNone(
			`SELECT
    ...
    FROM OPENJSON(@p3) WITH (
        ...
        [QtyAcc] money) as p`,
			[
				syncParams.project.id,
				syncParams.source.id,
				JSON.stringify(PositionPurchase),
			],
		);
		...
		if (!NoSqlDocument) {
			...
			if (syncParams.logLevel > 1) {
				await saveLogProtocol(
					...
					0,
					syncStage,
					`Purchase ${iikoDoc.id} #${iikoDoc.documentNumber} insert`,
				);
			}
			...
			if (syncParams.logLevel > 1) {
				await saveLogProtocol(
					...
					0,
					syncStage,
					`Purchase ${iikoDoc.id} #${iikoDoc.documentNumber} update`,
				);
			}
		}
		...
		NoSqlDocument.doc.Items = PositionPurchase;
...
export async function ImportPurchaseToJetti(
	syncParams: ISyncParams,
	docList: any[] = [],
) {
	await saveLogProtocol(
	...
	if (syncParams.baseType === 'sql')
		await ImportPurchaseSQLToJetti(syncParams, docList);
}
export async function ImportPurchaseSQLToJetti(
	syncParams: ISyncParams,
	docList: any[] = [],
) {
	...
	const newSQL = `SELECT id,
                    cast(comment as nvarchar(255)) as comment,
                    cast(conception as nvarchar(50)) as conception,
                    dateIncoming,
                    cast(documentNumber as nvarchar(33)) as documentNumber,
                    status,
                    cast(defaultStore as nvarchar(50)) as defaultStore,
                    cast(supplier as nvarchar(50)) as supplier,
                    incomingDate,
                    cast(incomingDocumentNumber as nvarchar(33)) as incomingDocumentNumber,
                    cast(revision as nvarchar(50)) as revision,
                    dateModified,
                    cast(userModified as nvarchar(50)) as userModified
                    FROM dbo.IncomingInvoice  ${sw}`;
	i = 0;
	let batch: IPurchaseInvent[] = [];
			...
			if (rowCount && !more && batch.length > 0) {
				if (syncParams.logLevel > 1)
					await saveLogProtocol(
						...
						0,
						syncStage,
						`inserting tail ${batch.length} Purshase docs`,
					);
				for (const doc of batch) {
					const docResult = transformPurshase(syncParams, doc);
					await syncInventSQL(syncParams, docResult, ssql, dsql);
