    ...
    GO
      ...
      CREATE OR ALTER VIEW dbo.[Catalog.TaxRates] AS
        
      SELECT
        d.id, d.type, d.date, d.code, d.description "TaxRates", d.posted, d.deleted, d.isfolder, d.timestamp
        , ISNULL("parent".description, '') "parent.value", d."parent" "parent.id", "parent".type "parent.type"
        , ISNULL("company".description, '') "company.value", d."company" "company.id", "company".type "company.type"
        , ISNULL("user".description, '') "user.value", d."user" "user.id", "user".type "user.type"
        , ISNULL([workflow.v].description, '') [workflow.value], d.[workflow] [workflow.id], [workflow.v].type [workflow.type]
        ...
        , ISNULL(l5.description, d.description) [TaxRates.Level5]
        , ISNULL(l4.description, ISNULL(l5.description, d.description)) [TaxRates.Level4]
        , ISNULL(l3.description, ISNULL(l4.description, ISNULL(l5.description, d.description))) [TaxRates.Level3]
        , ISNULL(l2.description, ISNULL(l3.description, ISNULL(l4.description, ISNULL(l5.description, d.description)))) [TaxRates.Level2]
        , ISNULL(l1.description, ISNULL(l2.description, ISNULL(l3.description, ISNULL(l4.description, ISNULL(l5.description, d.description))))) [TaxRates.Level1]
      FROM [Catalog.TaxRates.v] d WITH (NOEXPAND)
        LEFT JOIN [Catalog.TaxRates.v] l5 WITH (NOEXPAND) ON (l5.id = d.parent)
        LEFT JOIN [Catalog.TaxRates.v] l4 WITH (NOEXPAND) ON (l4.id = l5.parent)
        LEFT JOIN [Catalog.TaxRates.v] l3 WITH (NOEXPAND) ON (l3.id = l4.parent)
        LEFT JOIN [Catalog.TaxRates.v] l2 WITH (NOEXPAND) ON (l2.id = l3.parent)
        LEFT JOIN [Catalog.TaxRates.v] l1 WITH (NOEXPAND) ON (l1.id = l2.parent)
      
        LEFT JOIN [Catalog.TaxRates.v] [parent] WITH (NOEXPAND) ON [parent].id = d.[parent]
        LEFT JOIN dbo.[Catalog.User.v] [user] WITH (NOEXPAND) ON [user].id = d.[user]
        LEFT JOIN dbo.[Catalog.Company.v] [company] WITH (NOEXPAND) ON [company].id = d.company
        LEFT JOIN dbo.[Document.WorkFlow.v] [workflow.v] WITH (NOEXPAND) ON [workflow.v].id = d.[workflow]
    
      GO
      GRANT SELECT ON dbo.[Catalog.TaxRates] TO jetti;
      GO
