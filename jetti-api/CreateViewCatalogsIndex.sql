      ...
      CREATE OR ALTER VIEW dbo.[Catalog.TaxRates.v] WITH SCHEMABINDING AS
      SELECT id, type, date, code, description, posted, deleted, isfolder, timestamp, parent, company, [user]
      , TRY_CONVERT(UNIQUEIDENTIFIER, JSON_VALUE(doc, N'$."workflow"')) [workflow]
      , ISNULL(TRY_CONVERT(MONEY, JSON_VALUE(doc, N'$."Rate"')), 0) [Rate]
      FROM dbo.[Documents]
      WHERE [type] = 'Catalog.TaxRates'
    
      GO
      CREATE UNIQUE CLUSTERED INDEX [Catalog.TaxRates.v] ON [Catalog.TaxRates.v](id);
      
      CREATE UNIQUE NONCLUSTERED INDEX [Catalog.TaxRates.v.code.f] ON [Catalog.TaxRates.v](parent,isfolder,code,id) INCLUDE([company]);
      CREATE UNIQUE NONCLUSTERED INDEX [Catalog.TaxRates.v.description.f] ON [Catalog.TaxRates.v](parent,isfolder,description,id) INCLUDE([company]);
      CREATE UNIQUE NONCLUSTERED INDEX [Catalog.TaxRates.v.description] ON [Catalog.TaxRates.v](description,id) INCLUDE([company]);
      CREATE UNIQUE NONCLUSTERED INDEX [Catalog.TaxRates.v.code] ON [Catalog.TaxRates.v](code,id) INCLUDE([company]);
      CREATE UNIQUE NONCLUSTERED INDEX [Catalog.TaxRates.v.user] ON [Catalog.TaxRates.v]([user],id) INCLUDE([company]);
      CREATE UNIQUE NONCLUSTERED INDEX [Catalog.TaxRates.v.company] ON [Catalog.TaxRates.v](company,id) INCLUDE([date]);

      GRANT SELECT ON dbo.[Catalog.TaxRates.v] TO jetti;
      GO
      --------------------------------------------------------------------------------------
      ...
      ADD FILTER PREDICATE [rls].[fn_companyAccessPredicate]([company]) ON [dbo].[Catalog.TaxRates.v],
